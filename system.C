...
int check_connection_status(void) {
    ...
    if (!write_message_file("TestConnection")) {
        is_online = 0;
        return 0;
    }
    ...
    fgets(buffer1, sizeof(buffer1), f);  /* session ID line */
    fgets(buffer2, sizeof(buffer2), f);  /* message line */
    fclose(f);
    ...
    is_online = (strcmp(buffer2, "Online") == 0);
    return is_online;
}
...
int send_message_to_helper(const char* command_line) {
    if (!write_message_file(command_line)) {
...
int read_response_file(char lines[][MAX_LINE_LENGTH], int max_lines, int skip_first_line) {
    int count = 0;
    FILE* f;
    int len;

    if (testing_mode) {
        /* Insert your test-mode simulation logic here if needed */
        return 0;
    }
    ...
    if (skip_first_line) {
        ...
        fgets(temp, sizeof(temp), f);  /* discard first line */
    }

    while (fgets(lines[count], MAX_LINE_LENGTH, f) != NULL && count < max_lines) {
...
int start_chat_session(int agent_id) {
    ...
    if (!send_message_to_helper(cmd)) return 0;
...
void handle_chat(int agent_id) {
    ...
    if (!start_chat_session(agent_id)) {
        return;
    }
    ...
    while (in_chat) {
        ...
        gotoxy(2, 24);
        for (i = 0; i < CONSOLE_WIDTH; i++) cprintf(" ");
        gotoxy(2, 24);
        cprintf("> ");
        gotoxy(4, 24);
        ...
        if (i > 0 && in_chat) {
            ...
            if (send_message_to_helper(command)) {
                ...
                while (steps < max_steps) {
                    ...
                    gotoxy(30, 22);
                    cprintf("%s", line);
                    fflush(stdout);  /* ensures spinner is displayed */
                ...
                gotoxy(30, 22);
                cprintf("                    ");  /* Clear spinner */

                ui_show_cursor();

                /* Redraw screen again to display response cleanly */
                clrscr();
                textcolor(LIGHTGREEN);
                gotoxy(2, 1);
                cprintf("Chatting with: %s (ESC to end)", current_agent_name);
